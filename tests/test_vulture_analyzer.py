...
def test_scan_code_parses_vulture_output(monkeypatch):
    ...
    monkeypatch.setattr(
        VultureAnalyzer, "_check_vulture_installation", lambda self: None
    )
    ...
    monkeypatch.setattr("mcp_server_analyzer.analyzers.vulture.subprocess.run", fake_run)
    ...
    assert created_temp_paths and not created_temp_paths[0].exists()
